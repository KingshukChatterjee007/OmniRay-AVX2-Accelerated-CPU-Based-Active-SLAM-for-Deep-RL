...
\vspace{1pt}
{\raggedright\footnotesize $^\dagger$\,Inference decision latency (policy network pass takes $3.09$\,ms, full localization and mapping step takes $10.02$\,ms); full environment training loop step takes 14--26\,ms. Baseline latencies represent decision planning latency.\par}
\end{table}
...
The result ($91.08 \pm 3.99\%$) represents a slight $+0.21$ percentage-point increase in mean coverage relative to the Intel Lab ($90.87 \pm 3.29\%$). However, this gap is well within one standard deviation of both measurements, and no statistically significant difference was observed under a two-sample comparison. The primary scientific takeaway is that the generalization gap is within the noise margin, demonstrating that the trained policy generalizes directly to the unseen MIT Stata Center without performance degradation.
...
\vspace{1pt}
{\raggedright\footnotesize $^\dagger$\,Inference decision latency (policy network pass takes $3.09$\,ms, full localization and mapping step takes $10.02$\,ms). Baseline latencies represent decision planning latency.\par}
\end{table}
