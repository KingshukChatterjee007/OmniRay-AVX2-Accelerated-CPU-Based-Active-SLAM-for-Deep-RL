...
\begin{tabular}{lcccccc}
\toprule
\textbf{Model / Baseline} & \textbf{Coverage} & \textbf{Distance} & \textbf{Coverage / Meter} & \textbf{Decision Latency} & \textbf{Peak RAM} & \textbf{Collisions} \\
& (\%) & (m) & (\%/m) & (ms) & (MB) & Count \\
\midrule
...
\midrule
\textbf{OmniRay (Ours)} & $90.87 \pm 3.29\%$ & $418.12 \pm 106.25\,\text{m}$ & $0.2172\,\%/\text{m}$ & $\mathbf{3.09\,\text{ms}}^\dagger$ & $\mathbf{41.8\,\text{MB}}$ & $62.3 \pm 46.5$ \\
\bottomrule
\end{tabular}%
}
\caption{\textbf{Multi-Seed Exploration Benchmark on Intel Floorplan:} Comprehensive evaluation across coverage efficiency (\%/m), decision latency, peak RAM footprint, and collision safety under kinodynamic noise (Mean $\pm$ SD). $^\dagger$Refers to policy network decision latency ($3.09\,\text{ms}$ pass; full decision step including SLAM belief update is $10.02\,\text{ms}$). Core C++ AVX2 SIMD raycasting kernel latency is $0.038\,\text{ms}$ per scan; baseline latencies represent decision planning latency.}
\label{tab:baseline}
\end{table}
...
\midrule
\textbf{OmniRay (Zero-Shot)} & $91.08 \pm 3.99\%$ & $454.34 \pm 108.03\,\text{m}$ & $0.2005\,\%/\text{m}$ & $\mathbf{3.09\,\text{ms}}^\dagger$ & $\mathbf{41.8\,\text{MB}}$ & $70.3 \pm 53.6$ \\
\bottomrule
\end{tabular}%
}
\caption{\textbf{MIT Stata Center Zero-Shot Benchmark Comparison:} Zero-shot evaluation on the MIT Stata Center dataset without policy fine-tuning or environment hyperparameter retuning (Mean $\pm$ SD). $^\dagger$Refers to policy network decision latency ($3.09\,\text{ms}$ pass; full decision step including SLAM belief update is $10.02\,\text{ms}$). Core C++ AVX2 SIMD raycasting kernel latency is $0.038\,\text{ms}$ per scan; baseline latencies represent decision planning latency.}}
\label{tab:mit_stata}
\end{table}
