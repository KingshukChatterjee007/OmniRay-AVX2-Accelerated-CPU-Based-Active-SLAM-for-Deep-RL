...
\caption{\textbf{MIT Stata Center Zero-Shot Benchmark Comparison:} Zero-shot evaluation on the MIT Stata Center dataset without policy fine-tuning or environment hyperparameter retuning (Mean $\pm$ SD).}
\label{tab:mit_stata}
\end{table}

\paragraph{Cross-Dataset Generalization Gap} Evaluating OmniRay zero-shot on the complex MIT Stata Center atrium topology yields $91.08 \pm 3.99\%$ final map coverage compared to $90.87 \pm 3.29\%$ on the Intel Research Lab floorplan, resulting in a slight $+0.21$ percentage-point increase in mean coverage, demonstrating exceptionally robust zero-shot generalisation across unseen topologies.
